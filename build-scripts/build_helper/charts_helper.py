    ...
    if len(platform_chart.kaapana_collections) > 0:
        for collection_name, collection_chart in platform_chart.kaapana_collections.items():
            platform_params["kaapana_collections"].append(
                {"name": collection_chart.name, "version": BuildUtils.platform_build_version}
            )
    ...
    if len(platform_chart.preinstall_extensions) > 0:
        for preinstall_extension_name, preinstall_extension_chart in platform_chart.preinstall_extensions.items():
            platform_params["preinstall_extensions"].append(
                {"name": preinstall_extension_chart.name, "version": BuildUtils.platform_build_version}
            )
    ...
    if len(chart_object.chart_containers) > 0:
        ...
        for container_tag, container in chart_object.chart_containers.items():
            ...
            for base_image in container.base_images:
                ...
                tree.create_node(
                    f"{base_image.name}:{base_image.version}", base_img_node_id, parent=base_images_node_id
                )
...
class HelmChart:
    ...
    def add_dependency_by_id(self, dependency_name, dependency_version):
        ...
        if len(charts_found) > 1:
            ...
            charts_found = [
                x for x in charts_found if x.name == dependency_name and x.repo_version == dependency_version
            ]
            BuildUtils.logger.warning(
            ...
            if container_found.operator_containers != None:
                for operator_container in container_found.operator_containers:
                    oparator_container_found = [
                        x for x in BuildUtils.container_images_available if x.tag == operator_container
                    ]
                    ...
                    if "image:" in line:
                        ...
                        if "#" in line.split("image:")[0]:
                            ...
                            container_tag = container_tag.replace(
                                ".Values.global.registry_url", BuildUtils.default_registry
                            )
                            container_tag = container_tag.replace(
                                ".Values.global.kaapana_build_version", self.repo_version
                            )
    ...
    def make_package(self):
        ...
        output = run(
            command, stdout=PIPE, stderr=PIPE, universal_newlines=True, timeout=60, cwd=dirname(self.build_chart_dir)
        )
        ...
        with alive_bar(len(charts_found), dual_line=True, title="Collect-Charts") as bar:
            for chartfile in charts_found:
                ...
                if (
                    ...
                    and sum([ignore_pattern in chartfile for ignore_pattern in BuildUtils.build_ignore_patterns]) != 0
                ):
    ...
    def create_platform_build_files(platform_chart):
        ...
        with alive_bar(bar="classic", spinner="crab", dual_line=False, title="Generate Build-Version") as bar:
            HelmChart.create_chart_build_version(
                src_chart=platform_chart, target_build_dir=platform_build_files_target_dir, bar=bar
            )
    ...
    def create_collection_build_files(collections_chart, platform_build_files_target_dir):
        ...
        with alive_bar(bar="classic", spinner="crab", dual_line=False, title="Generate Build-Version") as bar:
            HelmChart.create_chart_build_version(
                src_chart=collections_chart, target_build_dir=collection_build_target_dir, bar=bar
            )
        with alive_bar(
            collections_chart.dependencies_count_all, dual_line=True, title="Generate Build-Version"
        ) as bar:
            for collection_chart_index, (collection_chart_name, collection_chart) in enumerate(
                collections_chart.dependencies.items()
            ):
                BuildUtils.logger.info(
    ...
    def create_chart_build_version(src_chart, target_build_dir, bar=None):
        ...
        for dep_chart_index, (dep_chart_key, dep_chart) in enumerate(src_chart.dependencies.items()):
            ...
            HelmChart.create_chart_build_version(
                src_chart=dep_chart, target_build_dir=dep_chart_build_target_dir, bar=bar
            )

            assert exists(dep_chart.build_chartfile)

        for chart_container_id, chart_container in src_chart.chart_containers.items():
            chart_container.build_tag = (
                f"{BuildUtils.default_registry}/{chart_container.image_name}:{BuildUtils.platform_build_version}"
            )
    ...
    def build_platform(platform_chart):
        ...
        repo_version, repo_branch, repo_last_commit, repo_last_commit_timestamp = BuildUtils.get_repo_info(
            platform_chart.chart_dir
        )
        ...
        if BuildUtils.create_offline_installation is True:
            ...
            OfflineInstallerHelper.export_image_list_into_tarball(
                image_list=successful_built_containers, images_tarball_path=images_tarball_path, timeout=4000
            )
