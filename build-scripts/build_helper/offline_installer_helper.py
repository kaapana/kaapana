...
class OfflineInstallerHelper:
    ...
    def download_gpu_operator_chart(target_path):
        ...
        output = run(
            ...
            timeout=OfflineInstallerHelper.HELM_DOWNLOAD_TIMEOUT,
        )
    ...
    def download_snap_package(name, version, target_path):
        ...
        output = run(
            ...
            stderr=PIPE,
            universal_newlines=True,
            timeout=OfflineInstallerHelper.SNAP_DOWNLOAD_TIMEOUT,
        )
        if output.returncode != 0:
            ...
            os.rename(
                assert_file_path, assert_file_path.replace(f"_{snap_version}", "")
            )

    @staticmethod
    def export_image_list_into_tarball(image_list, images_tarball_path, timeout=600):
        BuildUtils.logger.info(
            f"Exporting images as tarball @{images_tarball_path} ..."
        )
        BuildUtils.logger.warn(
            f"This can take a long time! -> please be patient and wait."
        )
        command = (
            [Container.container_engine, "save"]
            + [
                build_tag
                for build_tag in image_list
                if not build_tag.startswith("local-only")
            ]
            + ["-o", images_tarball_path]
        )
        output = run(
    ...
    def generate_microk8s_offline_version(build_chart_dir):
        ...
        images_tarball_path = join(
            microk8s_offline_installer_target_dir, "microk8s_base_images.tar"
        )
        ...
        OfflineInstallerHelper.export_image_list_into_tarball(
            image_list=microk8s_base_images, images_tarball_path=images_tarball_path
        )

        server_install_script_path = join(
            BuildUtils.kaapana_dir, "server-installation", "server_installation.sh"
        )
        assert exists(server_install_script_path)
        dst_script_path = join(
            microk8s_offline_installer_target_dir, basename(server_install_script_path)
        )
        copyfile(src=server_install_script_path, dst=dst_script_path)
        os.chmod(dst_script_path, 0o775)

        offline_enable_gpu_script_path = join(
            BuildUtils.kaapana_dir, "server-installation", "offline_enable_gpu.py"
        )
