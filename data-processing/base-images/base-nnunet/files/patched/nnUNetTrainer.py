...
class nnUNetTrainer(NetworkTrainer):
    ...
    def finish_online_evaluation(self):
        ...
        log_dict = {
            "loss/train": float(self.all_tr_losses[-1]),
            "loss/val": float(self.all_val_losses[-1]),
        }

        if len(self.all_val_eval_metrics) > 0:
            self.writer.add_scalar(
                f"foreground-dice/all", self.all_val_eval_metrics[-1], self.epoch
            )
            for idx, label_name in self.dataset_labels.items():
                if int(idx) == 0:  # label_name == 'Clear Label'
                    continue
                log_dict.update(
                    {
                        f"foreground-dice/label_{label_name}": float(
                            global_dc_per_class[int(idx) - 1]
                        )
                    }
                )
        ...
        log_dict.update(
            {"epoch": self.epoch, "fold": self.fold, "timestamp": time.time()}
        )
