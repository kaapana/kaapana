...
try:
    ...
    itk_zip_objects = [object_name for object_name in object_names if
                       object_name.endswith(".zip")]

    itk_directories = []
    for object_name in object_names:
        object_directory = str(Path(object_name).parents[0])
        if not object_directory.endswith(("imagesTr", "imagesTs", "labelsTr",
                                          "labelsTs", "cases", "segs")):
...
dag = DAG(dag_id="dag-convert-itk2dcm", default_args=args,
...
dcm_send_seg = DcmSendOperator(name="dcm-send-seg", dag=dag,
...
def branching_zipping_callable(**kwargs):
    download_dir = Path(AIRFLOW_WORKFLOW_DIR) / kwargs[
        'dag_run'].run_id / get_object_from_minio.operator_out_dir
