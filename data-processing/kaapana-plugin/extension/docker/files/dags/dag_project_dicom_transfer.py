    ...
    "workflow_form": {
        ...
        "properties": {
            "projects": {
                ...
                "type": "array",
                "items": {"type": "string", "enum": get_all_project_names()},
                "required": True,
            },
            "copy_dataset": {
...
class LocalCleanCustomTagsOperator(KaapanaPythonBaseOperator):
    ...
    def __init__(self, dag, **kwargs):
        super().__init__(dag=dag, name="clean-custom-dags", python_callable=self.clean, **kwargs)
        ...
        for batch_element_dir in batch_folder:
            ...
            for project_name in projects:
                ...
                try:
                    ...
                    log.info(f"Copied object '{minio_object_path}' to bucket '{target_bucket}'.")
...
assign_to_project = LocalAssignDataToProjectOperator(dag=dag, from_other_project=True, input_operator=clean_tags)
add_datasets = LocalAddToDatasetOperator(dag=dag, from_other_project=True, input_operator=clean_tags)
push_json = LocalJson2MetaOperator(dag=dag, json_operator=clean_tags, from_other_project=True)
...
(get_input >> clean_tags >> assign_to_project >> push_json >> add_datasets >> copy_thumbnails >> clean)
