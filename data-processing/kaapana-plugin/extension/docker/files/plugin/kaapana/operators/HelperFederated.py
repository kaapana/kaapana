...
def apply_minio_presigned_url_action(
    ...
    with requests.Session() as s:
        r = requests_retry_session(session=s).get(
            f"{MINIO_CLIENT_URL}/job", timeout=60, params={"job_id": client_job_id}
        )
    ...
    with requests.Session() as s:
        r = requests_retry_session(session=s).get(
            f"{MINIO_CLIENT_URL}/kaapana-instance",
            timeout=60,
            params={"instance_name": client_job["owner_kaapana_instance_name"]},
        )
    raise_kaapana_connection_error(r)
    ...
    if action == "put":
        ...
        if not os.path.isdir(src_dir):
            src_dir = os.path.join(root_dir, BATCH_NAME) # TODO should be passed from parents, because batch_name could be individual in some workflows
    ...
    if action == "get":
        ...
        with requests.Session() as s:
            with requests_retry_session(session=s, use_proxies=True).get(
                minio_presigned_url,
                verify=ssl_check,
                stream=True,
                timeout=600,
...
def federated_sharing_decorator(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if (
            ...
            and "federated_operators" in federated
        ...
        if (
            ...
            federated_action(
                ...
                federated,
                conf["client_job_id"],
                self.whitelist_federated_learning,
            )
