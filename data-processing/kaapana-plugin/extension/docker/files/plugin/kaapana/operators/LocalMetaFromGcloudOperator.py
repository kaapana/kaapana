...
class LocalMetaFromGcloudOperator(KaapanaPythonBaseOperator):
    """
    Operator to extract meta data from gcloud to json.
    ...
    * json file: output json file. DICOM tags are converted to a json file.
    ...
    def predict_modality(metadata_dict: dict):
        ...
        """
        assert LocalMetaFromGcloudOperator.MODALITY_TAG in metadata_dict
        modality = metadata_dict[LocalMetaFromGcloudOperator.MODALITY_TAG]

        metadata_dict.update({"00000000 CuratedModality_keyword": modality})
        if LocalMetaFromGcloudOperator.IMAGE_TYPE_TAG in metadata_dict:
            image_type = metadata_dict[LocalMetaFromGcloudOperator.IMAGE_TYPE_TAG]
    ...
    def dicomweb_search_instance(self, project_id, location, dataset_id, dicom_store_id):
    ...
    def start(self, ds, **kwargs):
        print("Starting module meta-from-gcloud...")
        print(kwargs)
        ...
        print("Extracting metadata from gcloud:")
        print(
            f"projects/{self.project_id}/locations/{self.location}/datasets/{self.dataset_id}/dicomStores/{self.dicom_store_id}")
        all_metadata_dict = self.dicomweb_search_instance(
        ...
        if len(all_metadata_dict) == 0:
            print("No metadata retrieved!")
            raise ValueError("ERROR")

        print("length: ", len(all_metadata_dict))
        for json_dict in all_metadata_dict:
            series_uid = json_dict["0020000E"]["Value"][0]

            target_dir: Path = run_dir / "batch" / series_uid / self.operator_out_dir
            target_dir.mkdir(exist_ok=True, parents=True)

            json_file_path = target_dir / f'{series_uid}.json'
    ...
    def get_time(self, time_str):
        try:
            ...
            time_formatted = parser.parse(
                time_string).strftime(self.format_time)
        ...
        for key, value in dicom_meta.items():
            ...
            if "vr" in value and "Value" in value:
                ...
                try:  # vr list: http://dicom.nema.org/dicom/2013/output/chtml/part05/sect_6.2.html
                    if vr in (
                        ...
                        try:
                            ...
                            print(
                                "Could not extract Date Time from: {}".format(
                                    value_str)
                            )
                            print(e)
                            if self.exit_on_error:
                ...
                if "InlineBinary" in value:
                    ...
                    if "Value" in value:
                        entry_value = str(value["Value"]).strip(
                            "[]").encode("utf-8")
    ...
    def cleanJsonData(self, path: Path):
        ...
        path_tmp: Path = path.parent / \
            path.name.replace(path.suffix, "_tmp.json")
        ...
        if modality_tag and modality_tag["Value"][0] in ("RTSTRUCT", "SEG"):
            label_results = LocalMetaFromGcloudOperator.get_label_tags(
                dicom_metadata)
        ...
        new_meta_data["00000000 TimestampArrived_datetime"] = self.convert_time_to_utc(
            timestamp_arrived.strftime(
                self.format_date_time), self.format_date_time
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not "GOOGLE_APPLICATION_CREDENTIALS" in os.environ:
            print("++++++++++++++++++++++++++++++++++++++++++++++++++++++")
            print("GOOGLE_APPLICATION_CREDENTIALS ENV NOT FOUND!")
        ...
        if not Path(os.environ["GOOGLE_APPLICATION_CREDENTIALS"]).exists():
            print("++++++++++++++++++++++++++++++++++++++++++++++++++++++")
            print("GOOGLE_APPLICATION_CREDENTIALS FILE IS NOT FOUND!")
            print("GOOGLE_APPLICATION_CREDENTIALS: {}".format(
                os.getenv("GOOGLE_APPLICATION_CREDENTIALS")))
            print("++++++++++++++++++++++++++++++++++++++++++++++++++++++")
            raise ValueError("ERROR")
