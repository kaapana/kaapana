...
class LocalDcm2JsonRefOperator:
    """
    ...
    * json file: output json file. DICOM tags are converted to a json file.
    ...
    def __init__(
        ...
        # TODO REMOVE
        self.airflow_workflow_dir = kwargs["workflow_dir"]
        self.batch_name = kwargs["batch_name"]
        self.operator_in_dir = kwargs["operator_in_dir"]
        self.operator_out_dir = kwargs["operator_out_dir"]
        # super().__init__(
        #     dag=dag,
        #     name="dcm2json",
    ...
    def start(self, ds, **kwargs):
        ...
        run_dir: Path = Path(self.airflow_workflow_dir,
                             kwargs["dag_run"].run_id)
        batch_folder: List[Path] = list(
            (run_dir / self.batch_name).glob("*"))
        ...
        for batch_element_dir in batch_folder:
            ...
            for dcm_file_path in dcm_files:
                ...
                # TODO REMOVE
                target_dir: Path = batch_element_dir.parent.parent / self.operator_out_dir
                target_dir.mkdir(exist_ok=True)
                json_file_path = target_dir / f"{dcm_file_path.stem}.json"
                ...
                dcm = pydicom.read_file(
                    dcm_file_path, stop_before_pixels=True)
    ...
    def _delete_pixel_data(self, dcm: pydicom.Dataset) -> pydicom.Dataset:
        ...
        pixel_data_elements = [(0x0014, 0x3080), (0x7fe0, 0x0008),
                               (0x7fe0, 0x0009), (0x7fe0, 0x0010)]
    ...
    def _process_annotation_tags(self, metadata: Dict) -> Dict:
        ...
        if segment_entries:
            ...
            for label_entry in segment_entries:
                alg_name = label_entry.get("00620009", {}).get("Value", [])[0]
                alg_type = label_entry.get("00620008", {}).get("Value", [])[0]
                value = label_entry.get("00620005", {}).get("Value", [])[0].replace(
                    ",", "-").strip() if "00620005" in label_entry else None

                if alg_name:
                    update_metadata["00620002 SegmentSequence_object_object"]["00620009 SegmentAlgorithmName_keyword"] = alg_name
                if alg_type:
                    update_metadata["00620002 SegmentSequence_object_object"]["00620008 SegmentAlgorithmType_keyword"] = alg_type
                if value:
                    annotation_label_list.append(value)
    ...
    def _normalize_tag(self, new_tag: str, vr: str, value_str: str, metadata: Dict) -> Dict:
        if vr in (
            "AE", "AS", "AT", "CS", "LO", "LT", "OB", "OW", "SH", "ST", "UC", "UI", "UN", "UT"
        ):
            new_tag += "_keyword"
            ...
            checked_val = self._process_sequence_value(
                value_str
            )
            if checked_val is not None:
                new_tag += "_object"
    ...
    # TODO still break down into smaller function like process_known_vr, etc.
        ...
        if self.IMAGE_TYPE_TAG in metadata:
            ...
            if isinstance(image_type, list):
                if (
                    ...
                    metadata.update(
                        {"00000000 CuratedModality_keyword": "XR"})
    ...
    def _process_time_tags(self, metadata: Dict) -> Dict:
        ...
        if "0008002A AcquisitionDateTime_datetime" in metadata:
            ...
            datetime_formatted = parser.parse(
                datetime_string).strftime(self.kaapana_datetime_format)

        # TODO NAIVELY! Expects BerlinTime datetime and convert to UTC
        ...
        metadata["00000000 DayOfWeek_integer"] = datetime.strptime(
            datetime_formatted, self.kaapana_datetime_format).weekday()
        ...
        if "00100030 PatientBirthDate_date" in metadata:
            ...
            if "00101010 PatientAge_keyword" in metadata:
                ...
                if patient_age_scan != age_meta:
                    logger.error(highlight_message(
                        "Patient AGE inconsistency"))
                    logger.error(
    ...
    def _format_datetime_value(self, new_tag, value_str):
        ...
        if validate_format(value_str, self.dcm_datetime_format):
            datetime_formatted = datetime.strptime(
                value_str, self.dcm_datetime_format).strftime(self.kaapana_datetime_format)
        else:
        ...
        try:
            if datetime_formatted is None:
                if len(value_str) > 8:
                    ...
                    datetime_formatted = parser.parse(
                        value_str).strftime(self.kaapana_datetime_format)
                else:
                    logger.info(
                        f"Trying to parse short date format with default time.")
                    date = parser.parse(value_str).date()
                    time = parser.parse("01:00:00").time()
                    datetime_formatted = datetime.combine(
                        date, time).strftime(self.kaapana_datetime_format)
    ...
    def _format_date_value(self, value_str):
        ...
        try:
            ...
            if isinstance(value_str, list):
                date_formatted = [parser.parse(date_str).strftime(
                    self.kaapana_date_format) for date_str in value_str if date_str != ""]
            elif isinstance(value_str, str):
                date_formatted = parser.parse(
                    value_str).strftime(self.kaapana_date_format)
            else:
                raise TypeError(
    ...
    def _get_time(self, time_str):
        ...
        time_formatted = parser.parse(
            time_string).strftime(self.kaapana_time_format)
...
def handle_incomplete_tag_metadata(tag_metadata: Dict):
    if "InlineBinary" in tag_metadata:
        ...
        if "Value" in tag_metadata:
            entry_value = str(tag_metadata["Value"]).strip(
                "[]").encode("utf-8")
