...
def convert_dcmseg(
    ...
    dcmqi_command = [
        ...
        "--outputType",
        output_type if output_type != "nii.gz" else "nii",
        "-p",
        f"{json_output}",
        "--outputDirectory",
        output_path,
    ...
    output = run(
        dcmqi_command, stdout=PIPE, stderr=PIPE, universal_newlines=True, timeout=120
    )
    ...
    try:
        ...
        logger.error(
            "DCMQI was not successfull in converting the dcmseg object. Might be due to missing resources!",
            e,
        )
        logger.error("Abort !")
        exit(1)
    ...
    for idx, segment in enumerate(meta_data["segmentAttributes"]):
        ...
        if (
            seg_filter is None
            or segment_label.lower().replace(",", " ").replace("  ", " ") in seg_filter
        ):
    ...
    if seg_filter != None and seg_filter != "":
        len_output_files = len(
            sorted(glob.glob(join(output_path, f"*{output_type}*"), recursive=False))
        )
        if len_output_files != len(seg_filter):
            logger.info(
                f"Found {len_output_files} files -> expected {len(seg_filter)}!"
            )
