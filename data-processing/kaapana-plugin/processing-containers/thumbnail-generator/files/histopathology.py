...
def generate_histopathology_thumbnail(
    ...
    if "OVERVIEW" in LuT:
        ...
        selected_file = None
        for i in range(start_index, len(dicom_files_tuples)):
            file, frames, rows = dicom_files_tuples[i]
            if rows * rows * frames < threshold_value:
                selected_file = file
                break
        # If no file was found, select the last file with fewer frames
        if not selected_file:
            selected_file = dicom_files_tuples[-1][0]
        ...
        if (
            ...
            if int(dcm_file.NumberOfFrames) == 1:
                ...
                reconstructed_image = np.zeros(
                    (
                        tiles_per_col * image_array.shape[1],
                        tiles_per_row * image_array.shape[1],
                        3,
                    ),
                ...
                b = image[x_min : x_max + 1, y_min : y_max + 1]
                pil_image = Image.fromarray(b)
        return pil_image.resize(size).convert("RGB")

    thumbnail = create_wsi_thumbnail(file, (thumbnail_size, thumbnail_size))
