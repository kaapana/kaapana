...
def train_val_datasplit():
    ...
    print(f"We are starting to split the data by the ratios: train_split={float(RATIO_TRAIN_SPLIT)}, val_split={ratio_val_split}")

    # load .csv-files from minio to metadata_all
    metadata_fnames = glob.glob(os.path.join('/minio', MINIO_OPERATOR_BUCKETNAME, '*.csv'), recursive=True)
