...
class LocalDoccanoDownloadDatasetOperator(KaapanaPythonBaseOperator):
    ...
    def start(self, ds, **kwargs):
        ...
        payload = {
            "username": "doccanoadmin",
            "password": "doccano123",
        }  # if password was set via the django api
        ...
        url = (
            DOCCANO_API
            + f'projects/{project_number}/download?taskId={download["task_id"]}'
        )
        download_url(url, os.path.join(batch_output_dir, "data.zip"), headers)

    def __init__(self, dag, name="doccano-download-dataset", *args, **kwargs):
        super().__init__(
            dag=dag, name=name, python_callable=self.start, *args, **kwargs
        )
