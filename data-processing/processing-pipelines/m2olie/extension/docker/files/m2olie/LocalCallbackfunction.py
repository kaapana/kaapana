    ...
    def start(self, ds, **kwargs):
        ...
        while time.time() < t_end:
            ...
            if r.status_code == 200:
                ...
                if r.status_code == 200:
                    applications = r.json()
                    release = next((item for item in applications if item['releaseName'] == release_name), None)
                    if release:
                        print(release)
                        print("Sending result to Prometheus!")
                        url = workflow_from["prometheus_uri"] + "/setState"
                        payload = {"url": release["link"] }
                        response = requests.post(url, params={"processInstanceLabel":  workflow_from["processInstanceLabel"],
                ...
                if r.status_code == 200:
                    ...
                    if release:
                        ...
                        if response.status_code == 500 or response.status_code == 404:
                            print("Something whent wrong, could not send the link to prometheus.")
