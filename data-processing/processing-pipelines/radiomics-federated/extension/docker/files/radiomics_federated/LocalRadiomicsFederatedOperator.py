...
class KaapanaFederatedTrainingBase(ABC):
    ...
    def get_conf(self):
        conf_data = self.conf.get('params')
    ...
    def __init__(self, workflow_dir=None,
        ...
        self.workflow_dir =  os.path.join(AIRFLOW_WORKFLOW_DIR, conf['dag_run'].run_id, BATCH_NAME, datetime.datetime.now().strftime("batch-%Y-%m-%d_%H-%M-%S"))  # workflow_dir or os.getenv('AIRFLOW_WORKFLOW_DIR', f'/kaapanasrc/data/federated-setup-central-test-220316153201233296')
        ...
        if self.use_minio_mount is None:
            self.fl_working_dir = os.path.join('/', self.workflow_dir, os.getenv('OPERATOR_OUT_DIR', output_dir))
        else:
    ...
    def download_minio_objects_to_workflow_dir(self, federated_round, tmp_central_site_info):
        ...
        for instance_name, tmp_site_info in tmp_central_site_info.items():
            ...
            print(f'Removing objects from previous federated_round_dir ({previous_federated_round_dir}) on Minio')
    ...
    def train(self):
        ...
        if self.use_minio_mount is not None:
            dst = os.path.join(self.fl_working_dir, os.path.basename(self.json_writer.filename))
            os.makedirs(os.path.dirname(dst), exist_ok=True)
...
class FederatedSetupTestFederatedTraining(KaapanaFederatedTrainingBase):
    ...
    def __init__(self, workflow_dir=None, use_minio_mount=None, conf=None, output_dir=None):
        super().__init__(workflow_dir=workflow_dir, use_minio_mount=use_minio_mount, conf=conf, output_dir=output_dir)
...
class LocalRadiomicsFederatedOperator(KaapanaPythonBaseOperator):

    def start(self, ds, **kwargs):
        kaapana_ft = FederatedSetupTestFederatedTraining(conf=kwargs, output_dir=self.operator_out_dir)#use_minio_mount='/minio')
        kaapana_ft.train()
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        super(LocalRadiomicsFederatedOperator, self).__init__(
           dag=dag,
           name=f'radiomics-federated-operator',
