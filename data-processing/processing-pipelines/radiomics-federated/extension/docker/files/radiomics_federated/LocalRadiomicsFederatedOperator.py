...
class KaapanaFederatedTrainingBase(ABC):
    ...
    def __init__(
        ...
        if self.use_minio_mount is None:
            self.fl_working_dir = os.path.join(
                "/", self.workflow_dir, os.getenv("OPERATOR_OUT_DIR", output_dir)
            )
        else:
            self.fl_working_dir = os.path.join(
            ...
            with requests.Session() as s:
                r = requests_retry_session(session=s).post(
                    ...
                    json={
                        ...
                        "status": "queued",
                        "addressed_kaapana_instance_name": self.client_network[
                            "instance_name"
                        ],
    ...
    def download_minio_objects_to_workflow_dir(
        ...
        for instance_name, tmp_site_info in tmp_central_site_info.items():
            ...
            print(
                f"Removing objects from previous federated_round_dir ({previous_federated_round_dir}) on Minio"
            )
    ...
    def train(self):
        ...
        if self.use_minio_mount is not None:
            dst = os.path.join(
                self.fl_working_dir, os.path.basename(self.json_writer.filename)
            )
            os.makedirs(os.path.dirname(dst), exist_ok=True)
...
class FederatedSetupTestFederatedTraining(KaapanaFederatedTrainingBase):
    def __init__(
        self, workflow_dir=None, use_minio_mount=None, conf=None, output_dir=None
    ):
        super().__init__(
            workflow_dir=workflow_dir,
            use_minio_mount=use_minio_mount,
            conf=conf,
...
class LocalRadiomicsFederatedOperator(KaapanaPythonBaseOperator):
    def start(self, ds, **kwargs):
        kaapana_ft = FederatedSetupTestFederatedTraining(
            conf=kwargs, output_dir=self.operator_out_dir
