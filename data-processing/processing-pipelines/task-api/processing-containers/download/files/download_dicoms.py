...
class GetInputOperator:
    def __init__(self):
        ...
        self.project = self.get_project(os.getenv("KAAPANA_PROJECT_IDENTIFIER"))
        self.project_index = self.project.get("opensearch_index")

        logger.info(self.project)

    def get_project(self, identifier: str):
        aii_url = os.getenv("KAAPANA_AII_URL")
        r = requests.get(f"{aii_url}/projects/{identifier}")
        r.raise_for_status()
        return r.json()
    ...
    def get_data(self, dcm_uid_object):
        """
        Download series data from PACS
    ...
    def get_identifiers(self, dataset: str):
        ...
        r = requests.get(
            ...
            params={"name": dataset},
            verify=False,
            headers={"Project": json.dumps(self.project)},
        )
    ...
    def download_data_for_workflow(self):
        ...
        parallel_downloads = int(os.getenv("PARALLEL_DOWNLOADS"))
...
if __name__ == "__main__":
    ...
    logger.info(f"{operator.project=}")
