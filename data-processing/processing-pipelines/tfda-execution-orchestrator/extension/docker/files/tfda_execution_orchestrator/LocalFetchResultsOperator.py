    ...
    def start(self, ds, ti, **kwargs):
        ...
        if not os.path.isfile(fetch_results_playbook_path):
            raise AirflowFailException(
                f"Playbook '{fetch_results_playbook_path}' file not found!"
            )
        ...
        logging.info(
            f"Fetching results from isolated execution environment for {workflow_type} workflow..."
        )
        ...
        if rc == 0:
            ...
            raise AirflowFailException(
                "Fetching results FAILED! Cannot proceed further..."
            )

    def __init__(self, dag, **kwargs):
        super().__init__(
            dag=dag, name="fetch-results", python_callable=self.start, **kwargs
        )
