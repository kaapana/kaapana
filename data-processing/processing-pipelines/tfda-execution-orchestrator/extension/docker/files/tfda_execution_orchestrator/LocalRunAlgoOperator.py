...
class LocalRunAlgoOperator(KaapanaPythonBaseOperator):
    def start(self, ds, ti, **kwargs):
        ...
        run_workflow_playbook_path = os.path.join(
            playbooks_dir, f"run_{workflow_type}.yaml"
        )
        if not os.path.isfile(run_workflow_playbook_path):
            raise AirflowFailException(
                f"Playbook '{run_workflow_playbook_path}' file not found!"
            )
        ...
        if rc == 0:
            ...
            raise AirflowFailException(
                "Running workflow FAILED! Cannot proceed further..."
            )

    def __init__(self, dag, **kwargs):
        super().__init__(
            dag=dag, name="run-isolated-workflow", python_callable=self.start, **kwargs
        )
