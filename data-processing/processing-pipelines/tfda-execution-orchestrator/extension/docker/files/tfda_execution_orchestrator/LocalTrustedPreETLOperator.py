...
class LocalTrustedPreETLOperator(KaapanaPythonBaseOperator):
    ...
    def algo_pre_etl(self, config_params):
        ...
        user_experiment_path = os.path.join(
            operator_dir,
            "algorithm_files",
            config_params["workflow_type"],
            config_params["experiment_name"],
        )
        ...
        if config_params["workflow_type"] == "shell_workflow":
            ...
            try:
                ...
                with open(
                    os.path.join(
                        user_experiment_path, f"{config_params['experiment_name']}.zip"
                    ),
                    "wb",
                ) as file:
            ...
            if return_code == 0:
                ...
                raise AirflowFailException(
                    "Login to the registry FAILED! Cannot proceed further..."
                )

            logging.debug(f"Pulling container: {container_name_version}...")
            tarball_file = os.path.join(
                user_experiment_path, f"{config_params['experiment_name']}.tar"
            )
            ...
            if return_code2 != 0:
                raise AirflowFailException(
                    f"Error while trying to download container! Exiting..."
                )
        else:
            raise AirflowFailException(
                f"Workflow type {config_params['request_type']} not supported yet! Exiting..."
            )
    ...
    def __init__(self, dag, **kwargs):
        super().__init__(
            dag=dag, name="trusted-pre-etl", python_callable=self.start, **kwargs
        )
