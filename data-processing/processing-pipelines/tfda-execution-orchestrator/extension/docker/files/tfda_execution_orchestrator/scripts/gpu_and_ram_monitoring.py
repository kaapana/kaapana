    ...
    if get_gpu_memory()[0] - 1104 > gpu_limit or ram_used > ram_limit:
        if get_gpu_memory()[0] - 1104 > gpu_limit:
            ...
            with open(medperf_logfile, "a") as logfile:
                logfile.write(
                    f"{now.strftime('%Y-%m-%d %H:%M:%S,%f')[:-3]} | ERROR: GPU Memory usage exceeded {gpu_limit} GB, value is: {(get_gpu_memory()[0]-1104)/1000} GB"
                )
        elif ram_used > ram_limit:
            now = datetime.now()
            with open(medperf_logfile, "a") as logfile:
                logfile.write(
                    f"{now.strftime('%Y-%m-%d %H:%M:%S,%f')[:-3]} | ERROR: RAM usage exceeded {ram_limit} GB, value is: {ram_used} GB"
                )
