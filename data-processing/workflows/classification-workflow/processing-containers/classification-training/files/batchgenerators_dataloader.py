...
# ("train" template: "preprocessed" input channel).
...
class ClassificationDataset(DataLoader):
    ...
    def generate_train_batch(self):
        ...
        for i, j in enumerate(patients_for_batch):
            input_image_path = os.path.join(PREPROCESSED_MOUNT, j, j + ".npy")
    ...
    def get_split(random_seed):
        ...
        if len(all_samples) == 0:
            raise ValueError("No images in path: %s" % PREPROCESSED_MOUNT)
