.. _api_documentation_root:

API Documentation
#################

Here we document the processing steps available in the repository. We have adopted the terminology of Apache Airflow:

.. warning::
   This area is currently under construction and does not list all available operators. 
   For the full picture, please have a look at the `kaapana.operators <https://github.com/kaapana/kaapana/tree/develop/data-processing/kaapana-plugin/extension/docker/files/plugin/kaapana/operators>`_ package in the code.

.. toctree::
    :maxdepth: 2

    api_documentation/api_dags
    api_documentation/api_operators
    