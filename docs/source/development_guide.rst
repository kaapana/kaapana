...
This development guide is intended to provide a quick and easy way to get started with developments on the platform
 
The guide currently consists of three parts. The parts TODO and TODO focus on the implementation of pipelines for Airflow in order to apply processing steps to images. The part TODO explains how to develop a flask web application and integrate it as an extensions into the Kaapana technology stack.
...
* Installation of `DCMTK <https://dicom.offis.de/dcmtk.php.en>`_ (optional: convienent way to send images to the platform)

**Upload images to the platform**
* Log in to the platform (here you find the credentials)
* Upload images:
* Either create a zip file of images that end with .dcm and upload the images viea drag&drop on the landing page in the section "Data upload"
...
* In order to create a development environment to add new DAGs to the platform go to the extension section on the landing page and install the code-server-chart. Clicking on the link you will be served with a Visual Studio Code Environment in the directory of Airlfow, where the Kaapana plugin is implemented ("workflows/plugins"), the data during processing ("workflows/data"), the models ("workflows/models") and the dags are stored ("workflows/dags"). 

In order to get a general idea about how to use the platform checkout TODO. Furthermore, it might be helpful to checkout the TODO in order to get an idea of the concepts of the Kaapana platform.

Write your fisrt own DAG
^^^^^^^^^^^^^^^^^^^^^^^^

**Aim**: Create a DAG that converts DICOMs to .nrrd files

In order to deploy now a new DAG that converst DICOMs to nrrds, create a file called dag_example_dcm2nrrd.py inside the "dags"-folder with the following content:
...
* Go to Airflow via the landing page and check if your newly added DAG example-dcm2nrrd appears under DAGs 
...
* If everything was successfull you can go to minio via the landing page -> store -> Minio where you will find a bucket called example-dcm2nrrd. Inside this folder you will find the .nrrd files of the selected images.
...
------------------------------------------
First of all, it is important that you script works. For this, we will simmulate the folder structure that you can expect on the platform and apply our algorithm locally to the files. In order to simulate the folder structure. Please go back to the Meta-Dashboard, select the files you want to develop with and trigger the DAG "explore-data-in-minio". This will download the selected images to a folder in Minio called "exploration". Pleas go to Minio, download the folder called "batch" and save the extracted content to a folder called "data". Now the "data" folder corresponds to the "data" folder that you have seen in the Airflow folder.

.. hint::

  | In case you algorithm works with .nrrd files you could simply download the batch folder that we generated in the example-dcm2nrrd folder

* Now we create the following python script and make sure that we have Pydicom installed:
...
In order to test the script we create uncomment the os.environ sections and adapt the WORKFLOW_DIR to the "data" location on our local file system. Then we execute the script. In the platform all the environment variables will be set automatically. If the algorithm runs without errors, the most difficult part is already done, we have a running workflow!

Step 2: Check if our scripts runs inside a Docker container
-----------------------------------------------------------

The next step is to put the algorithm into a Docker container and test if everything works as expected. For this we will put the extract_study_id.py in a folder called files, comment again the os.environ section and create the following Dockerfile next to the files dirctory:
...
.. hint::
  ...
  | Also here you can take our templates as a starting points.
...
* Run the docker, however specifing the environment variable as well as by mounting your local file system into the Docker container to a directory called data. This mount will also be made automatically on the platform.
...
Step 3: Create a DAG and Operator that applies the integrates the created Docker container
------------------------------------------------------------------------------------------
...
* In order to use this operator, create file called dag_example_extract_study_id.py with the following content inside the Dag directory:
...
In the templates_and_examples folder you will find even more example DAGs and Docker container!
...
When you go to http://localhost:5000 you should see congratulations message!

Since flask run is only for development, we use gunnicorn to run in production. gunicorn is started via the boot.sh bash script. To try it please run:
...
Now the application can be accessed via http://localhost:5000/hello-world. As you can see we adapted the application to run on the prefix path /hello-world. A requirement for any application running in our platform is that it runs  within its own subpath, otherwise, it is no possible to launch it within the platform.
...
Again you should find the application via http://localhost:5000/hello-world
...
No you should be able to deploy the platform. Go to the server, inside the templates directory and execute:

::

   kubectl apply -f .

If everything works well you should, the  docker container is started inside the Kubernetes cluster. When going to /hello-world in your platform, you should see the congratulations page again. Furthermore you should also be able to see the application running on the port 5000. This is, because we specified a NodePort in the service.yaml file.
...
Step 4: Write a helm chart and expose it as an extensions 
---------------------------------------------------------
...
No you should have the same result as before, when you created the deployment with the kubectl. With helm ls you can view all helm releases that are currently running.

In order to remoe the chart execute:
...
The hello-world-chart folder corresponds to a Chartmuseum, which we can easily :

* Add the helm-repo to which you want to push the data:

::
   
   helm repo add --username jip-ci-dcipher --password <password> <repo-name> https://dktk-jip-registry.dkfz.de/chartrepo/<repo-name>
