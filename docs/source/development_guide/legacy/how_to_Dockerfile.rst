...
Base images
-----------
Kaapana Dockerfiles typically utilize base images such as :code:`ubuntu:24.04` or :code:`python:3.9.16-slim`. However, it's strongly advised to use the pre-existing base images located under :code:`data-processing/base-images` in the Kaapana project. For tasks requiring Python, consider using the :code:`base-python-cpu` or :code:`base-python-gpu` images for CPU-bound and GPU-accelerated tasks, respectively. Only :code:`base-python-gpu` carries PyTorch (and :code:`kaapana_federated`, which imports it); an image that imports :code:`torch`, directly or through a library such as :code:`monai`, must build on it even when it runs on the CPU. :code:`base-python-cpu` ships no compilers, so an image that has to compile a Python package installs :code:`build-essential` itself, and no plotting libraries (plotly, seaborn, matplotlib), so an image that plots lists them in its own requirements. code-server is not part of any base image; the development server adds it at start-up.
...
.. code-block:: bash
  ...
  pip install -c https://codebase.helmholtz.cloud/kaapana/constraints/-/raw/0.7.0/constraints.txt <package-name>
...
.. code-block::
  ...
  RUN pip install --upgrade pip && \
      pip install -c https://codebase.helmholtz.cloud/kaapana/constraints/-/raw/0.7.0/constraints.txt -r requirements.txt
