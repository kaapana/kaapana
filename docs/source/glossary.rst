...
.. glossary::
    ...
    kaapanactl
      ...
      Administrators of a Kaapana Installation can use this script to

        #. Prepare the server to deploy kaapana by installing all required dependencies
        #. Deploy or undeploy a :term:`kaapana-platform` into a :term:`Kubernetes<kubernetes>` cluster by installing and configuring the :term:`kaapana-admin-chart` using :term:`Helm<helm>`
        #. Report generation that collects diagnostic information about the installation, useful when troubleshooting issues.
