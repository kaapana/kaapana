...
class Annotations:
    ...
    def resume_annotation_from_csv(self, df) -> None:
        for i in range(len(df["file"])):
            self.data[df["file"][i]] = Annotation(
                annotator=df["annotator"][i], time=df["time"][i], choice=df["choice"][i]
            )
...
app.layout = html.Div(
    [
        html.Div(id="dummy_load", children=[], style={"display": "none"}),
        html.Div(id="dummy_buttons", children=[], style={"display": "none"}),
        html.Div(id="dummy_update", children=[], style={"display": "none"}),
        html.H1(
            className="bg-info text-white p-2",
            children=[
                dbc.Row(
                    [
                        dbc.Col(["RateMe"], width={"size": 1, "offset": 0}),
                        dbc.Col(
                            [
                                html.Img(
                                    src=path_logo, height="70px"
                                ),  # ToDo: x-ray size can vary
                            ],
                    ...
                    [
                        ...
                        dbc.Popover(
                            [
                                ...
                                dbc.PopoverBody(
                                    "Overwrites the current annotation labels"
                                ),
                            ],
                            id="upload_csv_popover",
                            target="upload",
                            trigger="hover",
                            placement="bottom",
                        ),
                        ...
                        dbc.Popover(
                            [
                                ...
                                dbc.PopoverBody(
                                    "Downloads the current annotation values as .csv file"
                                ),
                            ],
                            id="download_annotations_popover",
                        ...
                        dbc.Card(
                            [
                                ...
                                dbc.CardBody(
                                    html.P(
                                        [
                                            "1. Set path to data folder and user ID on the left.",
                                            html.Br(),
                                            html.Br(),
                                            "2. Annotate image selection by using the buttoms.",
                                            html.Br(),
                                ...
                                html.Div(
                                    [
                                        dcc.RangeSlider(
                                            ...
                                            pushable=0.05,
                                            tooltip=dict(
                                                always_visible=True, placement="left"
                                            ),
                                            persistence_type="session",
                                            disabled=True,
                                            vertical=True,
                        ...
                        dbc.Row(
                            [
                                ...
                                dbc.Popover(
                                    ...
                                    id="current_file_popover",
                                    target="input_circular",
                                    trigger="hover",
                                    placement="bottom",
                                ),
                            ]
                    ...
                    [
                        ...
                        dbc.Popover(
                            [
                                ...
                                dbc.PopoverBody(
                                    "Select from list of un-annotated images"
                                ),
                            ],
                            id="check_annotations_popover",
                            target="missing_annotations_dropdown",
                            trigger="hover",
                            placement="top",
                        ),
                    ],
...
@app.callback(
    Output("missing_annotations_dropdown", "options"),
...
def update_files(_, data_path: str):
    ...
    if DATA.data_reader.n_images > 20:
        marks = {
            i + 1: str(i + 1)
            for i in range(
                0, DATA.data_reader.n_images, int(DATA.data_reader.n_images / 10)
            )
        }
    else:
...
def update_dummy(
    _, __, ___, ____, input_value: int, missing_annotation_id: int, file_index: int
):
    ctx = dash.callback_context
    input_id = ctx.triggered[0]["prop_id"].split(".")[0]
...
def update_accept_button(_, __, ___, ____, file_index: int):
    ...
    if input_id != "dummy_load" and input_id != "dummy_update":
        if input_id == "accept_button":  # or keydown["key"] == "a":
            DATA.data[file_name] = Annotation(
                annotator=DATA.user, time=datetime.now(), choice=1
            )
        if input_id == "reject_button":  # or keydown["key"] == "r":
            DATA.data[file_name] = Annotation(
                annotator=DATA.user, time=datetime.now(), choice=0
            )
        if input_id == "skip_button":  # or keydown["key"] == "s":
            DATA.data[file_name] = Annotation(
                annotator=DATA.user, time=datetime.now(), choice=np.nan
            )
