...
> `@kaapana/base-ui` Storybook. Design guidance lives under `Guidelines`; library
> components and APIs live under `Library`.
...
| `surface` | `#FFFFFF` | `#1E1E1E` | Cards, dialogs, and primary content surfaces |
| `surface-light` | `#F5F5F5` | `#383838` | Subtle grouping, toolbars, flat alerts, and input masks |
| `surface-bright` | `#FFFFFF` | `#424242` | High-emphasis surfaces above the main surface |
| `surface-variant` | `#424242` | `#C8C8C8` | Inverted surfaces such as tooltips, badges, chips, and snackbars |
| `error` | `#C62828` | `#EF5350` | Failure, invalid state, and destructive action |
| `warning` | `#EF6C00` | `#FFA726` | Attention required without implying failure |
| `success` | `#2E7D32` | `#66BB6A` | Successful completion or positive state |
| `info` | `#0277BD` | `#29B6F6` | Neutral information and status |
| `accent` | `#000000` | `#FFFFFF` | General high-contrast accent |

`surface-light` is a subtle step away from the main surface. `surface-variant` is
...
**Storybook:** `Guidelines / Foundations / Colors & Themes`
...
| `text-overline` | Short overline labels |
...
> **To be decided:** Decide on the spacing scale, whether the default corner
...
**Storybook:** `Guidelines / Foundations / Spacing & Shape`

### Icons

Use one recognizable symbol for each action across applications. Use the shared
semantic icon map instead of writing `mdi-*` names at call sites.
...
| `delete` | `mdi-delete` | Delete or remove an item |
| `edit` | `mdi-pencil` | Edit existing content |
| `error` | `mdi-alert-circle` | Indicate an error or failed state |
| `expand` | `mdi-chevron-down` | Expand or collapse content |
| `externalLink` | `mdi-open-in-new` | Open a destination in a new context |
| `help` | `mdi-help-circle-outline` | Provide contextual help |
| `info` | `mdi-information` | Indicate neutral information |
| `refresh` | `mdi-refresh` | Reload or refresh current data |
| `save` | `mdi-content-save` | Save changes |
| `search` | `mdi-magnify` | Search available content |
| `start` | `mdi-play` | Start or run an operation |
| `success` | `mdi-check-circle` | Indicate successful completion |

**Storybook:** `Guidelines / Foundations / Icons`
...
| **Destructive** | Actions that delete data, cancel work, or are difficult to undo |
...
**Storybook:** `Guidelines / Actions / Buttons`
...
Make the destructive action visually distinct by giving it the `error` color
variant, but never make it the initial focus.

**Storybook:** `Guidelines / Patterns / Actions Requiring Confirmation / Destructive`
...
State the scope and expected effect. Use the `primary` color for confirmation;
reserve `error` for destructive actions.
...
**Storybook:** `Guidelines / Patterns / Actions Requiring Confirmation / High Impact`
...
**Storybook:** `Guidelines / Dialogs / Sizes`
...
> on both sides, and do not end with a hyphen. For example: `lung-segmentation`.

Avoid:

> Invalid input.

**Storybook:** `Guidelines / Forms / Validation`
...
**Storybook:** `Guidelines / Feedback / Errors`
...
**Storybook:** `Guidelines / Feedback / Empty States`
