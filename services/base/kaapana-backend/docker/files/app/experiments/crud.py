...
def sync_client_remote(
    ...
    for db_incoming_job in db_incoming_jobs:
        db_incoming_experiment = get_experiments(
            db, experiment_job_id=db_incoming_job.id
        )
        incoming_experiment = (
            [
                schemas.Experiment(**experiment.__dict__).dict()
                for experiment in db_incoming_experiment
            ][0]
            if len(db_incoming_experiment) > 0
            else None
        )
    ...
    for db_remote_kaapana_instance in db_remote_kaapana_instances:
        ...
        for incoming_experiment in incoming_experiments:
            ...
            db_incoming_experiment = get_experiment(
                db, experiment_name=incoming_experiment["experiment_name"]
            )  # rather query via experiment_name than via experiment_id
...
def sync_states_from_airflow(db: Session, periodically=False):
    ...
    diff_curr_to_glob = [
        elem for elem in jobs_in_qsr_state if elem not in glob_jobs_in_qsr_state
    ]
    # find elements which are in glob_jobs_in_qsr_state from previous round but not in current jobs_in_qsr_state
    diff_glob_to_curr = [
        elem for elem in glob_jobs_in_qsr_state if elem not in jobs_in_qsr_state
    ]
    ...
    if len(diff_curr_to_glob) > 0:
        ...
        for diff_job in diff_curr_to_glob:
            ...
            db_job = get_job(
                db, run_id=diff_job["run_id"]
            )  # fails for all airflow jobs which aren't user-created aka system-jobs
...
def create_experiment(db: Session, experiment: schemas.ExperimentCreate):
    ...
    if (
        ...
        .filter_by(experiment_name=experiment.experiment_name)
        .first()
...
async def queue_generate_jobs_and_add_to_exp(
    db: Session,
