...
class KeycloakHelper:
    ...
    def __init__(self, server_url: str, kaapana_admin_user: str, kaapana_admin_passsword: str, kaapana_realm: str = "kaapana", verify: bool=False):
    ...
    def assign_client_roles(self, user_id: str, client_name: str, role_names: List[str]):
        ...
        result = self.keycloak_admin.assign_client_role(client_id=client_id, user_id=user_id, roles=role_ids)
        if not result:
...
def main():
    ...
    if backend_user_id:
        log.info(f"Backend user already inplace id: {backend_user_id}, skipping user creation...")
    else:
