...
describe('menu store polling pauses while the tab is hidden', () => {
  let hidden = false
  ...
  function startSpiedPolling() {
    const menu = useMenuStore()
    const refresh = vi.spyOn(menu, 'refresh').mockResolvedValue()
    vi.spyOn(useProjectStore(), 'refreshProjects').mockResolvedValue()
    menu.startPolling()
    return refresh
  }

  it('skips the ticks of a hidden tab', () => {
    const refresh = startSpiedPolling()
  ...
  it('refreshes once on return, then resumes the cadence', () => {
    const refresh = startSpiedPolling()
    hidden = true
    vi.advanceTimersByTime(45_000)
