...
async def create_workflow_run(
    ...
    db_workflow_run = await crud.create_workflow_run(
        db, workflow_run=workflow_run, workflow_id=workflow_id
    )
    ...
    print(
        f"Created workflow run {db_workflow_run.id} with Celery task {celery_task.id}"
    )
...
def test_airflow_connection(forwarded_headers: Dict[str, str]) -> bool:
    logging.warning(
        f"Testing Airflow connection with forwarded headers: {forwarded_headers}"
    )
    adapter = celery.get_adapter_for_workflow(
        workflow_type="airflow", forwarded_headers=forwarded_headers
    )
    adapter.base_url = "http://airflow-webserver-service.services.svc:8080/flow"
    logging.warning(
        f"Testing Airflow connection with extra_headers: {adapter.extra_headers}"
    )
...
async def cancel_workflow_run(
    db: AsyncSession, forwarded_headers: Dict[str, str], workflow_run_id: int
) -> bool:
    """Cancel workflow run"""
    ...
    if workflow_run.external_id and workflow_run.lifecycle_status not in [
        LifecycleStatus.CANCELED,
        LifecycleStatus.COMPLETED,
    ]:
        celery.cancel_workflow.delay(
            forwarded_headers=forwarded_headers,
