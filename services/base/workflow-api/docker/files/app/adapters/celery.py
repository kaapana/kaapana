import logging
from app.adapters.config import settings, celery_app
...
def get_adapter_for_workflow(
    workflow_type: str, forwarded_headers: dict
) -> "WorkflowEngineBase":
    """
    ...
    Returns:
        WorkflowEngineBase: An instance of the appropriate adapter.
    Raises:
...
def submit_workflow_run(
    ...
    adapter: WorkflowEngineBase = get_adapter_for_workflow(
    ...
    try:
        ...
        logging.info(
            f"Submitting workflow run {workflow_run_id} to Airflow with config: {config}"
        )
        result = adapter.trigger_workflow_run(
            workflow_run_id, workflow_identifier, config, labels
        )
        ...
        with sync_session() as db:
            db.execute(
                ...
                .where(models.WorkflowRun.id == workflow_run_id)
                .values(
                    lifecycle_status=LifecycleStatus.ERROR, updated_at=datetime.now()
                )
            )
            db.commit()
...
def monitor_workflow(
    ...
    try:
        ...
        with sync_session() as db:
            ...
            if not workflow_run:
                logging.info(
                    f"Workflow run {workflow_run_id} not found in DB, stopping monitoring."
                )
            ...
            if current_status in [
                ...
                logging.info(
                    f"Workflow run {workflow_run_id} is already in a final state ({current_status.value}), stopping monitoring."
                )
                return {"status": current_status.value, "final": True}

            # Get the latest status from the external engine
            external_status = adapter.get_workflow_run_status(
                workflow_identifier, external_id
            )
            logging.info(
                f"Workflow run {workflow_run_id} (external ID: {external_id}) - DB status: {current_status.value}, External status: {external_status.value}"
            )
            ...
            if external_status != current_status:
                db.execute(
                    update(models.WorkflowRun)
                    .where(models.WorkflowRun.id == workflow_run_id)
                    .values(lifecycle_status=external_status, updated_at=datetime.now())
                )
                db.commit()
                logging.info(
                    f"Workflow run {workflow_run_id} status updated to {external_status.value}."
                )
            ...
            if external_status in [
                ...
                logging.info(
                    f"Workflow run {workflow_run_id} reached final state ({external_status.value}), stopping monitoring."
                )
            ...
            logging.info(
                f"Workflow run {workflow_run_id} monitoring rescheduled for {MONITOR_INTERVAL} seconds."
            )
        ...
        logging.info(
            f"Error monitoring workflow run {workflow_run_id}: {e}. Retrying..."
        )
        raise self.retry(exc=e, countdown=60, max_retries=3)
    ...
    with sync_session() as db:
        ...
        for task_data in tasks_info:
            ...
            task = (
                ...
                .filter_by(workflow_id=workflow_id, task_identifier=task_identifier)
                .first()
            )
            if not task:
                logging.info(
                    f"Task {task_identifier} not found in DB, creating new Task entry."
                )
                task = models.Task(
            ...
            task_run = (
                ...
                .filter_by(workflow_run_id=workflow_run_id, task_id=task.id)
                .first()
            )
        ...
        if all_statuses and all(status in final_states for status in all_statuses):
            logging.info(
                f"All tasks for workflow run {workflow_run_id} reached final states, stopping monitoring."
            )
...
def cancel_workflow(
    ...
    try:
        with sync_session() as db:
            ...
            logging.info(
                f"Attempting to cancel workflow {workflow_identifier} with external ID: {external_id}"
            )
            ...
            if success:
                ...
                logging.info(
                    f"Failed to cancel workflow {external_id} in external engine."
                )
