...
async def get_workflow_run_by_id(
    ...
    if run.lifecycle_status not in [
        schemas.LifecycleStatus.COMPLETED,
        schemas.LifecycleStatus.ERROR,
    ]:
        workflow = await crud.get_workflows(
            db,
            filters={"title": run.workflow.title, "version": run.workflow.version},
            single=True,
        )
        engine = get_workflow_engine(workflow)
        workflow_run_update = engine.get_workflow_run(run)
        run = await crud.update_workflow_run(
            db, run_id=run.id, workflow_run_update=workflow_run_update
        )
