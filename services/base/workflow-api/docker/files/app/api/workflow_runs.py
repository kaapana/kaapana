...
async def get_runs(
    ...
    for db_workflow_run in runs:
        db_workflow = await crud.get_workflows(
            db,
            filters={"id": db_workflow_run.workflow_id},
            single=True
        )

        if db_workflow.project_id != UUID(project_id):
            raise HTTPException(status_code=403, detail="Workflow Run does not belong to this project")
...
async def get_workflow_run_task_runs(run_id: int, project_id=Depends(get_project_id), db: AsyncSession = Depends(get_async_db)):
    ...
    if task_runs is None:
        raise HTTPException(status_code=404, detail="No task runs found for this workflow run")
    #TODO is project check needed?
    return task_runs


@router.get("/runs/{run_id}/tasks/{task_id}/task-run", response_model=schemas.TaskRun)
async def get_task_run(run_id: int, task_id: int, project_id=Depends(get_project_id), db: AsyncSession = Depends(get_async_db)):
    #TODO is project check needed?
    db_task_run = await crud.get_task_run_by_workflow_run_and_task(db, task_id=task_id, workflow_run_id=run_id)
...
async def get_task_run_logs(run_id: int, task_id: int, project_id=Depends(get_project_id), db: AsyncSession = Depends(get_async_db)):
    db_workflow_run = await crud.get_workflow_runs(
        db,
        filters={"id": run_id},
        single=True,
...
@router.get("/workflows/{identifier}/versions/{version}/runs", response_model=List[schemas.WorkflowRun])
async def get_runs_by_workflow(
    ...
    if db_workflow is None:
        raise HTTPException(status_code=404, detail=f"Workflow not found in project {project_id}")
    
    # Get runs for this specific workflow
    runs = await crud.get_workflow_runs(
        db,
        filters={"workflow_id": db_workflow.id},
        skip=skip,
        limit=limit
    )
...
@router.post("/workflows/{identifier}/versions/{version}/runs", response_model=schemas.WorkflowRun)
