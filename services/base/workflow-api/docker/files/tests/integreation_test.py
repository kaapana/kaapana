...
async def create_workflow(headers, cookies, payload):
    async with httpx.AsyncClient(base_url=BASE_URL, verify=False) as client:
        response = await client.post("/workflow-api/v1/workflows", headers=headers, cookies=cookies, json=payload)
...
async def test_get_workflows(auth_headers_and_cookies):
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, verify=False) as client:
        response = await client.get("/workflow-api/v1/workflows", headers=headers, cookies=cookies)
        assert response.status_code == 200
...
async def test_get_workflow_by_identifier_and_version(auth_headers_and_cookies):
    ...
    data = await create_or_get_workflow(headers, cookies, payload)
...
async def test_get_latest_version_workflow(auth_headers_and_cookies):
    headers, cookies = auth_headers_and_cookies
    payload = {
        "identifier": "example-workflow-latest",
        "definition": "A test workflow",
        "config_definition": {"key": "value"},
    }
    data = await create_or_get_workflow(headers, cookies, payload)
...
async def test_get_workflow_versions(auth_headers_and_cookies):
    headers, cookies = auth_headers_and_cookies
    response = await httpx.AsyncClient(verify=False).get(
        f"{BASE_URL}/workflow-api/v1/workflows/example-workflow/versions",
        headers=headers, cookies=cookies
    )
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
async def test_get_all_runs(auth_headers_and_cookies):
    ...
    headers, cookies = auth_headers_and_cookies
    async with httpx.AsyncClient(base_url=BASE_URL, verify=False) as client:
        response = await client.get("/workflow-api/v1/runs", headers=headers, cookies=cookies)
        assert response.status_code == 200
...
async def test_get_runs_for_workflow(auth_headers_and_cookies):
    ...
    # 1. Create a workflow
    workflow = await create_or_get_workflow(headers, cookies, workflow_payload)
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, verify=False) as client:
        ...
        run_response = await client.post(
            ...
            headers=headers, cookies=cookies, json=run_payload
        )
        assert run_response.status_code == 200
...
async def test_get_task_run_details_and_logs(auth_headers_and_cookies):
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, verify=False, timeout=120) as client:
        ...
        assert task_run_found, f"Could not find or retrieve task-run for task '{task_id_in_definition}' in run '{run_id}'"
...
async def test_workflow_ui_schema(auth_headers_and_cookies):
    ...
    headers, cookies = auth_headers_and_cookies
    workflow_payload = {
        "identifier": "ui-schema-workflow-test",
        "definition": "A test workflow for UI schema",
        "config_definition": {"key": "value"},
    }
