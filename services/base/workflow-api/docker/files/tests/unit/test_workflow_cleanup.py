...
async def _ensure_workflow(session: AsyncSession) -> models.Workflow:
    """Get-or-create the canonical workflow used by the cleanup tests.
    ...
    """
    existing = await crud.get_workflow(session, filters={"title": "cleanup-wf"})
    if existing:
        return existing
    ...
    for title in ("dummy-task-1", "dummy-task-2"):
        session.add(models.Task(workflow_id=workflow.id, title=title, type="test"))
    await session.commit()
    return workflow
...
async def _make_workflow_and_run(
    ...
    *,
    ...
    workflow = await _ensure_workflow(session)
