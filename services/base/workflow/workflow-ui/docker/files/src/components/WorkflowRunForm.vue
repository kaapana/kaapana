...
const toggleTask = (name: string) => {
    ...
    vueformSchema.value = buildVueformSchema(workflow.value)
}
...
watch(isOpen, async (newValue) => {
    if (newValue) {
        ...
        resetForm()
        // detach keyboard shortcuts when dialog closes
...
const loadForm = async () => {
    ...
    try {
        ...
        if (props.workflow.parameters && Object.keys(props.workflow.parameters).length > 0) {
            groupedParams.value = props.workflow.parameters
            ...
            // fallback to legacy config_definition.workflow_parameters array
            const configDef = (props.workflow as any).config_definition
            if (configDef && configDef.workflow_parameters && configDef.workflow_parameters.length > 0) {
                const grouped = groupByTask(configDef.workflow_parameters)
                groupedParams.value = grouped
                Object.keys(grouped).forEach(k => {
                    if (taskOpen.value[k] === undefined) taskOpen.value[k] = true
                })
                vueformSchema.value = buildVueformSchema(grouped)
            } else {
...
function buildVueformSchema(params: Record<string, WorkflowParameter[]>): Record<string, any> {
    ...
    console.debug("Building Vueform schema")
