...
LABEL VERSION="17.11.2022"
WORKDIR /
ARG username
ARG password
## ssl error due to certificate problems with our git
RUN git config --global http.sslVerify false
#build container with: docker build --build-arg username=Username --build-arg password=password -t local/avid-base:17.11.2022 -f ./no.Dockerfile .
