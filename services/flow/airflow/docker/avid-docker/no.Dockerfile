...
LABEL VERSION="18.01.2023"
WORKDIR /
ARG username
ARG password
## ssl error due to certificate problems with our git
RUN git config --global http.sslVerify false
#build container with: docker build --build-arg username=Username --build-arg password=password -t local/avid-base:18.01.2023 -f ./no.Dockerfile .
RUN git clone https://"$username":"$password"@phabricator.mitk.org/diffusion/AVID/avid.git ./avid-src  && \
	...
	#works->git checkout 'c05b341bcea1'
	git checkout  master
################################################################################################################################
###################################################### Second Stage #########################################################
################################################################################################################################
