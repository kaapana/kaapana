...
wait_for_deployment() {
    ...
    # Explicit error message: under `set -e` a silent `kubectl get` failure
    # would abort the whole job without any hint which deployment is missing.
    if ! kubectl get deploy -n "$namespace" "$deployment" >/dev/null 2>&1; then
        echo "ERROR: deployment $namespace/$deployment not found."
        return 1
    fi
...
migrate_legacy_aii_admin_roles() {
    ...
    # Fresh (non-legacy) platforms never had an 'admin' role: current AII
    # only seeds 'scientist' and 'principal-investigator'. With no admin
    # role AND no legacy mappings there is simply nothing to migrate - this
    # must not fail the job on every clean install.
    if [[ "$admin_role_count" -eq 0 && "$legacy_count" -eq 0 ]]; then
        echo "No legacy 'admin' role present and no legacy mappings found - nothing to migrate."
        return 0
    fi
