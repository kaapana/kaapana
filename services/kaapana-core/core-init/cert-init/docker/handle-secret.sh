...
function install_cert_files {
    CERT_FILE=$1
    KEY_FILE=$2
    if ! kubectl --insecure-skip-tls-verify=true -n $SECRET_NAMESPACE create secret tls $SECRET_NAME --key $KEY_FILE --cert $CERT_FILE; then
        echo "ERROR creating secret $SECRET_NAME in namespace $SECRET_NAMESPACE" 
        exit 1
    fi
