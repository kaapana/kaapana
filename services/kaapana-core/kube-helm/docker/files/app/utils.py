...
def helm_show_values(name, version):
    try:
        chart = subprocess.check_output(
            f'{os.environ["HELM_PATH"]} show values {app.config["HELM_EXTENSIONS_CACHE"]}/{name}-{version}.tgz',
...
def helm_repo_index(repo_dir):
    ...
    subprocess.check_output(helm_command, stderr=subprocess.STDOUT, shell=True)
...
def helm_show_chart(name=None, version=None, package=None):
    ...
    if package is not None:
        ...
        helm_command = (
            f'{helm_command} {app.config["HELM_EXTENSIONS_CACHE"]}/{name}-{version}.tgz'
        )
    try:
        chart = subprocess.check_output(
...
def helm_get_manifest(release_name, namespace):
    try:
        manifest = subprocess.check_output(
            f'{os.environ["HELM_PATH"]} -n {namespace} get manifest {release_name}',
            stderr=subprocess.STDOUT,
            shell=True,
        )
    except subprocess.CalledProcessError as e:
...
def helm_get_values(release_name, namespace):
    try:
        values = subprocess.check_output(
            f'{os.environ["HELM_PATH"]} -n {namespace} get values -o json {release_name}',
            stderr=subprocess.STDOUT,
            shell=True,
        )
    except subprocess.CalledProcessError as e:
...
def helm_status(release_name, namespace):
    try:
        status = subprocess.check_output(
            f'{os.environ["HELM_PATH"]} -n {namespace} status {release_name}',
            stderr=subprocess.STDOUT,
            shell=True,
        )
    except subprocess.CalledProcessError as e:
...
def helm_prefetch_extension_docker():
    ...
    for chart_name, chart in extensions.items():
        if (
            os.path.isfile(f'{app.config["HELM_EXTENSIONS_CACHE"]}/{chart_name}.tgz')
            is not True
        ):
        ...
        if "kaapanaexperimental" in chart["keywords"]:
            ...
            try:
                release, _ = helm_install(
                    payload,
                    app.config["NAMESPACE"],
                    helm_command_addons="--dry-run",
                    in_background=False,
                )
    ...
    for dag in dags:
        try:
            ...
            helm_install(
                dag, app.config["NAMESPACE"], helm_comman_suffix=helm_comman_suffix
            )
        except subprocess.CalledProcessError as e:
            print(f'Skipping {dag["name"]} due to {e.output.decode("utf-8")}')
            helm_delete(
                release_name=dag["release_name"],
                namespace=app.config["NAMESPACE"],
                release_version=chart["version"],
...
def pull_docker_image(
    release_name, docker_image, docker_version, docker_registry_url, timeout="120m0s"
):
    ...
    with open(
        os.path.join(app.config["HELM_HELPERS_CACHE"], "index.yaml"), "r"
    ) as stream:
        try:
    ...
    helm_install(
        payload,
        app.config["NAMESPACE"],
        helm_comman_suffix=helm_comman_suffix,
        helm_cache_path=app.config["HELM_HELPERS_CACHE"],
    )
...
def helm_install(
    ...
    if http_proxy is not None and http_proxy != "":
        default_sets.update(
            {"global.http_proxy": http_proxy, "global.https_proxy": http_proxy}
        )
...
def helm_ls(namespace, release_filter=""):
    try:
        resp = subprocess.check_output(
            f'{os.environ["HELM_PATH"]} -n {namespace} --filter {release_filter} ls --deployed --pending --failed --uninstalling -o json',
            stderr=subprocess.STDOUT,
            shell=True,
        )
...
def check_modified():
    ...
    helm_packages = [
        f for f in glob.glob(os.path.join(app.config["HELM_EXTENSIONS_CACHE"], "*.tgz"))
    ]
    modified = False
    new_charts_hashes = {}
...
def helm_search_repo(keywords_filter):
    ...
    if check_modified() or charts_cached == None:
        ...
        helm_packages = [
            f
            for f in glob.glob(
                os.path.join(app.config["HELM_EXTENSIONS_CACHE"], "*.tgz")
            )
        ]
        charts_cached = {}
        for helm_package in helm_packages:
...
def get_kube_status(kind, name, namespace):
    states = {"name": [], "ready": [], "status": [], "restarts": [], "age": []}
...
def get_extensions_list():
    ...
    extensions_list = []
    try:
        if update_running or (
            not smth_pending
            and last_refresh_timestamp != None
            and extensions_list_cached
            ...
            available_charts = helm_search_repo(
            ...
            for name, versions in chart_version_dict.items():
                ...
                extension["experimental"] = (
                    "yes" if "kaapanaexperimental" in extension["keywords"] else "no"
                )
                extension["multiinstallable"] = (
                    "yes"
                    if "kaapanamultiinstallable" in extension["keywords"]
                    else "no"
                )
                    ...
                    for version in extension["versions"]:
                        if release["chart"] == f'{extension["name"]}-{version}':
                            manifest = helm_get_manifest(
                                release["name"], app.config["NAMESPACE"]
                            )
                            ...
                            running_extension["successful"] = all_successful(
                                set(kube_status["status"] + [release["status"]])
                            )
                            running_extension["installed"] = "yes"
                            running_extension["links"] = ingress_paths
                            running_extension["helmStatus"] = release[
                                "status"
                            ].capitalize()
                            running_extension["kubeStatus"] = ", ".join(
                                kube_status["status"]
                            )
                            ...
                            if running_extension["successful"] != "yes":
                                print(
                                    f"Chart {release['name']} not ready: {running_extension['successful']} -> pending...",
                                    flush=True,
                                )
                                smth_pending = True
