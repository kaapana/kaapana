...
class ProviderAPIEndpoints(BaseModel):
    identifier: str = Field(
        ..., description="An identifier to distinguish endpoints if there are multiple."
    )
...
class ProviderRegistration(BaseModel):
    ...
    api_endpoints: List[ProviderAPIEndpoints] = Field(
        ..., description="All API endpoints of the extension.", min_items=1
    )


class ProviderRegistrationOverview(TypedDict):
    id: str
    name: str
    url: str
...
class InternalProviderRegistration(ProviderRegistration):
    ...
    def get_overview(self) -> ProviderRegistrationOverview:
