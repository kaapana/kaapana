...
export const AddSegmentRow: React.FC<{ children?: React.ReactNode }> = ({ children = null }) => {
  ...
  const activeSegmentIndex = activeSegmentation
    ? Number(
        Object.keys(activeSegmentation.segments).find(
    ...
    <div className="my-px flex min-h-7 w-full items-center justify-between rounded pl-0.5 pr-7">
      <div className="mt-1 flex flex-row items-center gap-1">
        {allowAddSegment ? (
          ...
          >
            <Icons.Add />
            Add Segment <span className="ml-1 text-xs opacity-50">[M]</span>
          </Button>
        ) : null}
        {canReset ? (
          <Button
          ...
          >
            ...
            Reset Segment <span className="ml-1 text-xs opacity-50">[R]</span>
          </Button>
        ) : null}
        {onTogglePromptsVisibility ? (
          <Button
          ...
          >
            ...
            <span className="ml-1 text-xs opacity-50">[O]</span>
          </Button>
        ) : null}
      </div>
      <Button
