...
# Airflow log prefix "[2026-07-13T10:00:00.123+0000] ...", offset captured
...
def split_phases(client: KaapanaClient, dag_id: str, run_id: str, ti: dict) -> dict:
    ...
    queue: queued_when -> start_date, setup: start_date -> 'Creating pod',
    spawn: 'Creating pod' -> first container output, code: that -> end_date.
    Tasks without a pod creation get their whole runtime as 'code'.
    """
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if POD_CREATE in line:
                ...
                # first non-poll line after pod creation = container output
    ...
    for run in runs:
        ...
        for ti in tis:
            if ti.get("start_date") and ti.get("end_date"):
                task_phases[ti["task_id"]].append(split_phases(client, dag_id, run["dag_run_id"], ti))
                done[ti["task_id"]] = ti
        s, e = _ts(run.get("start_date")), _ts(run.get("end_date"))
        if s and e:
            dag_durations.append((e - s).total_seconds())
            # run duration minus the union of the task intervals
            intervals = sorted((_ts(t["start_date"]), _ts(t["end_date"])) for t in done.values())
            busy = 0.0
