"""GPU suite: run gpu_burn as a pod and parse throughput/verdicts from its logs."""
...
def run(kubectl: str, namespace: str, seconds: int = 60) -> dict:
    logs = run_pod(kubectl, manifest(namespace, seconds), POD_NAME, namespace, timeout_s=seconds + 3600)
    gflops = [float(m) for m in re.findall(r"\(([\d.]+) Gflop/s\)", logs)]
    # progress lines report "errors: 0 - 0", one number per GPU
    errors = [int(n) for m in re.findall(r"errors:((?:\s+\d+(?:\s+-)?)+)", logs) for n in re.findall(r"\d+", m)]
    if not gflops:
