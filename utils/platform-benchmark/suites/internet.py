"""Internet suite: run a speedtest pod and parse its --json output."""
...
def run(kubectl: str, namespace: str, proxy: str | None, timeout_s: int = 3600) -> dict:
    logs = run_pod(kubectl, manifest(namespace, proxy), POD_NAME, namespace, timeout_s)
    line = next(x for x in reversed(logs.splitlines()) if x.startswith("{"))
    r = json.loads(line)
    return {
