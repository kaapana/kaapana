...
# Namespace prefix of the platform (develop deployments create project
# namespaces as <prefix>-project-<id>). Empty matches legacy unprefixed
...
Data recovery options:
  ...
  --platform-prefix PREFIX       Namespace prefix of the platform; project
                                 namespaces are matched as
                                 [<prefix>-]project-* (default: empty)
...
run_recover() {
    ...
    # Forward the namespace prefix so project-namespace matching in
    # recover_data.sh also covers <prefix>-project-* namespaces.
    if [[ -n "${PLATFORM_PREFIX}" ]]; then
        recover_cmd+=(--platform-prefix "${PLATFORM_PREFIX}")
    fi
...
verify_state() {
    ...
    # Informational only: list both legacy unprefixed and prefixed project
    # namespaces (develop creates them as <prefix>-project-<id>).
    ${KUBE} get ns | grep -E "^(${PLATFORM_PREFIX}-)?project-" || true
}
...
main() {
    ...
    echo "Platform prefix:       ${PLATFORM_PREFIX:-<none>}"
