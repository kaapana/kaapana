...
wait_for_aii_projects_json() {
    ...
    while true; do
        ...
        error_summary="$(tail -n 1 "${error_file}" "${output_file}" 2>/dev/null | tail -n 1)"
...
derive_expected_project_namespaces() {
    local projects_json="$1"
    printf '%s\n' "${projects_json}" \
    | jq -r '.[] | select((.kubernetes_namespace // "") != "" and (.kubernetes_namespace | startswith("project-"))) | .kubernetes_namespace' \
    | sort -u
}
...
if [[ -z "${EXPECTED_NAMESPACES}" ]]; then
    echo "ERROR: AII returned no project-* namespaces."
    exit 1
fi
...
if [[ -n "${EXPECTED_NAMESPACES}" ]]; then
    ...
    while true; do
        ...
        if (( SECONDS >= validation_deadline )); then
            ...
            if printf '%s\n' "${MISSING_PVC_REFS[@]}" | grep -qx 'project-admin/tensorboard-pv-claim'; then
                echo "HINT: project-admin/tensorboard-pv-claim is missing."
            ...
            if printf '%s\n' "${MISSING_SECRET_REFS[@]}" | grep -qx 'project-admin/project-user-credentials'; then
                echo "HINT: project-admin/project-user-credentials is missing."
                echo "      The project-admin chart install may have failed or is still pending."
                echo "      Re-run reconciliation after kube-helm/init-projects are healthy."
            fi

            if printf '%s\n' "${MISSING_SECRET_REFS[@]}" | grep -qx 'project-admin/oidc-client-secret'; then
                echo "HINT: project-admin/oidc-client-secret is missing."
