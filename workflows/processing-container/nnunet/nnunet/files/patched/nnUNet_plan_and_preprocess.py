...
def main():
    ...
    parser.add_argument("--increment_step", type=str, default='all',
                        help="'all', 'to_dataset_properties' or 'from_dataset_properties', enables to do the"
    ...
    if args.increment_step not in ['all', 'to_dataset_properties', 'from_dataset_properties']:
        raise NameError("--increment_step has to be one of 'all', 'to_dataset_properties', 'from_dataset_properties'")
    tl = args.tl
    tf = args.tf
    ...
    for i in task_ids:
        ...
        if args.verify_dataset_integrity and args.increment_step in ['all', 'to_dataset_properties']:
            verify_dataset_integrity(join(nnUNet_raw_data, task_name))
        else:
            print('Skipping verify_dataset_integrity')
    ...
    for t in tasks:
        ...
        if args.increment_step in ['all', 'to_dataset_properties']:
            ...
            print('Skipping analzing the dataset')

        if args.increment_step == 'to_dataset_properties':
            print('Interupting plan and preprocess')
            return
